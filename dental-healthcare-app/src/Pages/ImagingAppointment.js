...
function ImagingAppointment() {
    ...
    useEffect(() => {
        const fetchImagingCenters = async () => {
            try {
                const [response] = await Promise.all([
                    axios.get('http://localhost:8000/api/imaging_center/sendimagingcenters/', {
                        headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:8000/api/imaging_center/imagingappointment/', formData, {
                headers: {
                    'Authorization': `Token ${token}`,
                    ...
                    <div className="flex flex-col justify-center px-6 py-7 bg-white rounded-lg shadow-md max-md:px-5 max-md:max-w-full">
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col mt-9 max-md:max-w-full">
                            ...
                            <div className="flex flex-col justify-center py-0.5 text-sm font-medium leading-5 text-gray-700 max-md:max-w-full">
                                <label className="justify-center self-start">Select Center</label>
                                <select
                                    name="center"
                                ...
                                >
                                    ...
                                    {centers.map((center) => (
                                        <option key={center.center_id} value={center.name}>{center.name} </option>
                                    ))}
                                </select>
                            </div>
...
export default ImagingAppointment;
